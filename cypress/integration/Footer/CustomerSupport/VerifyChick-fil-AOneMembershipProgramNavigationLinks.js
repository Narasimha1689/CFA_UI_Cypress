...
Then('Check Chick-fil-A One Membership Navigation links', (dataTable) => {
    dataTable.hashes().forEach(elem => {
        cy.get('ul.nav-links').eq(0).find('li > a').then((membershipElemList) => {
            expect(membershipElemList.text()).to.contain(elem.navLinks)
        });
    });
});
...
Then('Check Paragraph Text displayed on right for each left pane navigation items', (dataTable) => {
    dataTable.hashes().forEach((elem, index) => {
        cy.get('.history-toggle li').find('a').eq(index).then($getElemText => {
            ...
            if(getElemText.includes(elem.leftPaneNavItems)) {
                ...
                cy.get('.history-module .content', {timeout : 3000}).find('p').eq(index).should('contain', elem.paragraphText)
            }
        })             
    });
 });
